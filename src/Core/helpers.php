...
// ------------------------------------------------------------------------------------------------------------------
/**
 * Checks if te timezone is valid
 *
 * @param string $timeZone
 *
 * @return bool true, if the timezone is valid, otherwise false
 *
 */
function isTimeZoneValid(string $timeZone): bool
{
...
// ------------------------------------------------------------------------------------------------------------------
/**
 * Checks if te locale is valid
 *
 * @param string $locale
 *
 * @return bool true, if the locale is valid, otherwise false
 *
 */
function isLocaleValid(string $locale): bool
{
...
// ------------------------------------------------------------------------------------------------------------------
/**
 * Checks if te datetime is valid
 *
 * @param string $dateTime
 *
 * @return bool true, if the datetime is valid, otherwise false
 *
 */
function isDateTimeValid(string $dateTime): bool
{
    try {
...
// ------------------------------------------------------------------------------------------------------------------
/**
 * Converts a message to on choise for the autocomplete reult list
 *
 ...
 *
 * @return array the choise for the autocomplete list
 *
 */
function optionChoise(string $message): array
{
    return [ 'name' => $message, 'value' => $message ];
}

/**
 * Removed the slash command from the discord registry
 *
 * Sometime the DiscordPhp package fails to automaically remove/update the unused/removed/disabled commands,
 *  or does not update it if the structure is changed (ex.: option is csnged from required to optional)
 *  this function will remove it, after the bot should be restarted,
 *  after the updated command will be regitered again.
 *
 * ```php
 * deleteSlashCommand('listall','Was just for testing, no longer needed.');
 * ```
 *
 * @param string $name The name of the command
 * @param string $reason='' The reason sent to the discord registry for audit logging (if supported)
 *
 * @return void
 *
 */
function deleteSlashCommand(string $name, string $reason=''): void {

    $x = discord()->application->commands->find(function (Command $command) use ($name, $reason) {
        if ($command->name === $name) {
            discord()->application->commands->delete($command->id, $reason)
            ->done(fn ($result) =>
                DevLogger::warning('Deleted slash command', [
                    'name' => $name,
                    'reason' => $reason,
                    'result' => $result
                ])
